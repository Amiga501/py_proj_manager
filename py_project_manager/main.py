# -*- coding: utf-8 -*-
"""
Created on Thu Jan 30 21:53:28 2025

@author: brendan

"""
# Starting to remove the unused parts of cheche-pm to a minimal core, then 
# build back up.
...
class Project():
    """!
    The main class holding a project definition
    """
    ...
    def __init__(
            ...
            a_desc: list = None,
            a_duration: list = None,
            a_cost: list = None,
            a_precedence: list = None,
            a_resources: list = None,
            max_resources: list = None,
            G: dict = None,  # TODO - This could be a set, will determine later
            dummies: bool = False,
            ):
        """!
        **Instantiate the class**
        
        @param [in] n_activities [int] The number of activites within the 
            Project - this can be added to later
        @param [in] n_resources [int] The number of resources within the 
            project - will retire this in favour of specifiying resource by 
            name later and holding a register
        @param [in] activities [list] A list of the activies within the project
            - this can be added to later
        @param [in] a_desc [list] descriptions of each activity 
        @param [in] a_duration [list] duration of each activity
        @param [in] a_cost [list] cost (monetary??) of each activity
        @param [in] a_precedence [list] of precedences per activity
        @param [in] a_resources [list] a list of resources [not sure on definition ???]
        @param [in] max_resources [list] ???
        @param [in] G [dict] ???
        @param [in] dummies [bool]
        """
        # TODO - There are a number of items declared in instantiation that
        # don't really make sense. Unless the user has this class wrapped 
        # behind a loader that can read a json description for example, then 
        # create the Project from that.
        
        # Probably better to pass activies as alist of dictionary items rather
        # than disconnected lists, then if they need to be unzipped, do so in
        # the Project
    ...
    def add_activity(
            ...
            activity_precedence: list[str],
            a_desc: str = 'Activity',
            activity_cost: int | float = 0,
            activity_resources: int | float | list[int] | list[float] = -1,
            ):
        """!
        ...
        if activity_name in self.activities:
          raise ValueError(
              f'Activity {activity_name} already in project activities '
              f'{self.activities}')
        ...
        if activity_resources != -1:
          if self.n_resources == 0:
            ...
            if len(activity_resources) != self.n_resources:
              raise ValueError(
                  f'The number of resources does not match the number of '
                  'resources recorded for previously created activities '
                  f'({self.n_resources})')
            else:
              self.a_resources.append(activity_resources)
    ...
    def delete_activity(
        ...
        - The activity counter is decremented, and the activity index list is 
        ...
        if activity_name not in self.activities:
            raise ValueError(
                f'Activity {activity_name} not found in project activities '
                f'{self.activities}')
        else:
    ...
    def add_dummies_create_project_network(self):
        ...
        ------
        ValueError
            If 'Start' and 'End' nodes (dummies) are already added to the 
            project.

        Notes
        -----
        - This method extends the project network by adding 'Start' and 'End' 
            nodes and connecting them to the existing activities based on their 
            precedence.
        ...
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        def one_hot_resources(lista: list) -> list:
            output = []
            for x in lista:
                output += x
    ...
    def from_csv(cls, 
        ...
        """
        # TODO - retire this in favour of a JSON loader - and move it outside 
        # this class, the loader doesn't need to be here, 
        
        max_resources = max_resources or []
        ...
        if 'description' in df.columns:
          ...
          description = (['Start of Project'] 
                         + [f'Activity_{x}' for x in activities] 
                         + ['End of Project'])
        ...
        return cls(n_activities = n,
                   n_resources = n_r,
                   activities=activities,
                   a_desc = description,
                   a_duration = durations,
                   a_cost = cost,
                   a_precedence = precedence,
    ...
    # -------------------------------------------------------------------------
    def plot_network_diagram(self, plot_type: str = 'dot'):
        """!
        ...
        Notes
        -----
        The generated network diagram is saved as 'network_diagram.jpg' in the
        current directory. It is also displayed as a plot.
        ...
        if not self.dummies_added:
            warnings.warn(
                "!WARNING: Creating dummy variables and creating the project "
                "network...")
    ...
    def create_project_dict(self):
        ...
        """
        Creates an internal dictionary stored as an instance attribute with
        all the project relevant data.
    
        This method generates an internal dictionary that stores project-
        related information. The dictionary is stored as an instance attribute 
        'PROJECT' and will be used by other methods.
    ...
    def plot_gantt_cpm(self, 
        ...
        for i,activity in enumerate(CPM_SCHEDULE):
            ...
            if early:
                plt.broken_barh(
                    [(CPM_SCHEDULE[activity]['ES'], 
                      CPM_SCHEDULE[activity]['D'])],
                    (y_start-i, 0.75),
                    alpha=0.5,
                    color='red' if CPM_SCHEDULE[activity]['F'] == 0 else 'blue',
                    edgecolor='black',
                    )
                ...
                plt.broken_barh(
                    [(CPM_SCHEDULE[activity]['LS'],
                      CPM_SCHEDULE[activity]['D'])],
                    (y_start-i, 0.75),
                    alpha=0.5,
                    color='red' if CPM_SCHEDULE[activity]['F'] == 0 else 'purple',
                    edgecolor='black',
                    )
    ...
    def plot_resource_levels(self,solution = None,max_resources = None):
        ...
        if self.n_resources > 1:
            ...
            for i in range(R):
                ...
                axis[i].axhline(y = capaRessources[i]+0.02,
                                color = 'red',
                                linestyle= 'dashed',
                                lw=4,
                                )
        ...
        if self.n_resources == 1:
            ...
            plt.axhline(y = capaRessources[0]+0.02, 
                        color = 'red',
                        linestyle= 'dashed',
                        lw=4,
                        )
    ...
    # -------------------------------------------------------------------------
    @staticmethod
    def check_activity_day_schedulable(
            date,
            activity,
            PROJECT,
            PSG_SCHEDULE,
        ...
        """!
        **Class method to check activity**
    ...
    # no necesariamente tiene que estar adentro de la clase    
    # -------------------------------------------------------------------------
    def generate_datetime_schedule(self,
                                   solution = None,
                                   start_date = None,
                                   date_format = '%Y-%m-%d',
                                   weekends_work = False,
    ...
    def plot_date_gantt(self,
        ...
        if plot_type == 'plotly':
            ...
            fig_gantt_cpm.update_yaxes(
                autorange="reversed",
                )
            fig_gantt_cpm.update_layout(
                xaxis_title="Date",
    ...
    # -------------------------------------------------------------------------
    def run_all_pl_heuristics(self,
                              max_resources = None,
                              plot_type = 'plotly',
                              ):
        """!
    ...
    def genetic_algorithm_optimization(
        ...
        """!
        **Implements a genetic algorithm to solve the RCPSP **
        
        (Resource-Constrained Project Scheduling Problem).
    
        This method utilizes a genetic algorithm to find an optimal solution 
        ...
        as well as the SSG (Serial Schedule Generation) and PSG (Parallel 
        Schedule Generation) scheduling algorithms.
        The algorithm returns the best solution found after a specified number 
        of iterations.
    
        For references see:
          - Taillard, Éric D. Design of heuristic algorithms for hard 
              optimization: with python codes for the travelling salesman 
              problem. Springer Nature, 2023.
          - Ferrante Neri, Carlos Cotta, and Pablo Moscato. "Handbook of 
              memetic algorithms." Studies in Computational Intelligence. 
              Springer (2011)
          - "Heuristics for Scheduling Resource-Constrained Projects: An 
              Experimental Investigation," Dale F. Cooper, 1976,
            Management Science, https://doi.org/10.1287/mnsc.22.11.1186
          - Vanhoucke, Mario. Project management with dynamic scheduling. 
              Springer Berlin Heidelberg, 2012. (Chapter 7)
          - Artigues, Christian, Sophie Demassey, and Emmanuel Neron, eds.
              Resource-constrained project scheduling: models, algorithms, 
              extensions and applications. John Wiley & Sons, 2013. (Chapter 6)
          - Demeulemeester, Erik Leuven, and Willy S. Herroelen. Project 
              scheduling: a research handbook. Vol. 49. Springer Science & 
              Business Media, 2006. (Chapter 6)
        ...
        def Fitness(solution):
            ...
            if is_PL_valid(PL):
                if schedule_method == 1:
                    return ph/self.SSG(PL,
                                       max_resources=max_resources,
                                       verbose=False,
                                       save=False,
                                       )['End']['EF']
                else:
                    return ph/self.PSG(PL,
                                       max_resources=max_resources,
                                       verbose=False,
                                       save=False,
                                       )['End']['EF']
    ...
    # -------------------------------------------------------------------------
    @staticmethod
    def check_descendants_check_weight(
            Set,
            network_graph,
...
# -----------------------------------------------------------------------------
class UnusedProjectCode:
    """!
    **Code that was not accessed in Project, or is temporarily removed**
    
    """
    
    # -------------------------------------------------------------------------
    @classmethod
    def from_cv_dataset_rcp_file(
            cls,
            file_name,
            verbose=False,
            ):
        ...
        N,n_resources = list(map(
            int, 
            lines[0].replace('      ',',').split(',')[0:-1]))
        max_resources_list = list(map(
            int,
            lines[1].replace('      ',',').split(',')[0:-1]))
        ...
        end_node = str(N)
        if verbose:
            print(f'{file_name} Instance with {N} activities, Number of '
                  f'renewable resources = {n_resources}, max resources = '
                  f'{max_resources_list}')
            print()
        ...
        if lines[3][0] == ' ':
            ...
            dummy_0 = dummy_0[:-1] + lines[3].replace(
                '      ',',').replace(' ','').split(',')[:-1]
    ...
    def get_feasible_subset(
        ...
        for x in tqdm(power_set):
            prece_evaluation = self.check_descendants_only(
                x,
                network_graph,
            ...
            if prece_evaluation:
                ...
                resource_evaluation = self.check_check_weight_only(
                    x,
                    resource_consumption,
                    max_resources,
                    start_idx_0=start_idx_0,
        ...
        # Iterate through the sorted list and add non-subset tuples to the 
        # result
    ...
    # -------------------------------------------------------------------------
    def get_minimal_forbidden_sets(
            self,
            verbose=False,
            start_idx_0=True,
            set_str=False,
        ...
        Power_Set = []
        for x in tqdm(power_set):
            evaluation = self.check_descendants_check_weight(
                x,
                network_graph,
                resource_consumption,
                max_resources,
        ...
        if not str_output: 
            # Generate output in index format and not in str. 
            # Specially relevant for .rcp files
              forbidden_sets = [self.return_str_set_index_format(x) 
                                for x in forbidden_sets]
    
        return forbidden_sets
    
    # -------------------------------------------------------------------------
    def get_naive_graph_ES_EC_LS_LC(
            self,
            ph = None,
            verbose = True,
            ):
        ...
        for a in PROJECT:
            ...
            if verbose:
                print(f'Max start path: {max_start_path} with distance '
                      f'{max_start_dist}')
            ...
            for path in end_paths:
                ...
                if path_D > max_end_dist:
                    ...
                    max_end_dist = path_D
            if verbose:
                print(f'Max end path: {max_end_path} with distance '
                      f'{max_end_dist}')
    ...
    def get_weak_forbidden_sets(self, verbose=False):
        ...
        for i in range(n_r):
            ...
            for a in range(n):
                if u[a][i] > (B/2):                  
                  if verbose:
                      print(f'* Activity {activities[a]}, resource consumption'
                            f' {i+1} = {u[a][i]}, max capcity = {B}. '
                            f' {u[a][i]} > {B/2:0.2f}')
        ...
        for i in range(n_r):    
            if len(IS[i]) < 2:
              if verbose:
                  print('There is just one activity in the forbiddent set '
                        f' {i}. At least two activities are needed to '
                        'establish a constraint. The set will be substitued '
                        'by the empty set []')
              IS[i] = []
    ...
    # -------------------------------------------------------------------------
    def monte_carlo_cpm_detailed(self,
                                 pessimistic, 
                                 optimistic, 
        ...
        for j,a in enumerate(PROJECT):
            for i in range(NumberIterations):
                if a == 'Start' or a=='End':
                    ...
                    ActDurations[j,i] = np.random.triangular(
                        PROJECT[a]['optimistic'],
                        PROJECT[a]['duration'],
    ...
    # -------------------------------------------------------------------------
    def monte_carlo_cpm_simple(
            self,
            pessimistic: float = 0.25, 
            optimistic: float = 0.25, 
            NumberIterations: float = 1000,
            ):
        """!
    ...
    # -------------------------------------------------------------------------
    def monte_carlo_detail_buffer_analysis(
            self,
            pessimistic, 
            optimistic, 
            buffer, #
    ...
    # -------------------------------------------------------------------------
    def monte_carlo_simple_buffer_analysis(
            self,
            pessimistic = 0.25, 
            optimistic = 0.25, 
            buffer = None, 
